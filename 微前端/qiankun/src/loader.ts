...
export async function loadApp<T extends ObjectType>(
  ...
  const { entry, name: appName } = app; // 用户配置的入口和用户配置的名字
  ...
  // 会通过entry(子应用的入口)拿到用户的模板和执行脚本  包含js和css
  // get the entry html content and script executor
  // 会把js单独拿出粗来 template是注释掉外链的html
  // execScripts 代表用户引入的js
  ...
  // 创建shadowDom 通过创建一个元素 attchShowShadow
  ...
  // 把我们app的包裹容器，因为这个容器可能是一个shadowDOM.如果是shadowDOM，就需要拿到shadow中的dom
  ...
  // 执行用户的脚本,将global作为新的window
  const scriptExports: any = await execScripts(global, sandbox && !useLooseSandbox);
  // 从脚本中获取声明周期导出结果 bootstrap  mount....
